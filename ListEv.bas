...
Sub Activity_Create(FirstTime As Boolean)
	...
	CLV.Clear
	imgMap.Clear
	Core.ExecuteUrlGet(Main.server&"event", "LoadEV", Me)
End Sub
...
Private Sub CreateItem (id As String, name As String, date As String, img As String) As B4XView
	...
	Dim imgPath As String = Main.baseServer & "media/photos/" &img
	Log(img)
				...
				Try
					...
					If data.Size-1 > 0 Then
						...
						For i = 0 To data.Size-1
							Dim ev As Map
							ev = data.Get(i)
							CLV.Add(CreateItem(ev.Get("id"), ev.Get("name"), ev.Get("date_start"), ev.Get("gallery")), "")
